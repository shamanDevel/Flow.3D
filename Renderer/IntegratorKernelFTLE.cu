...
__global__ void computePathFTLEKernel(SimpleParticleVertexDeltaT* dpParticles, VolumeInfoGPU c_volumeInfo, BrickIndexGPU c_brickIndex, BrickRequestsGPU c_brickRequests, uint particleCount)
{
	...
	float time2texScale;
	if (!findBrickTime(c_volumeInfo, c_brickIndex, c_brickRequests, vertex.Position, vertex.Time,
	...
	while (step < c_integrationParams.stepCountMax && vertex.Time < brickTimeMax)
	{
		...
		if(stepAccepted) 
		{
			...
			if (!isInBrickTime(vertex.Position, vertex.Time, brickBoxMin, brickBoxMax, brickTimeMin, brickTimeMax)) 
			{
				...
				if (isOutOfDomain || !findBrickTime(c_volumeInfo, c_brickIndex, c_brickRequests, vertex.Position, vertex.Time,
														brickBoxMin, brickBoxMax, world2texOffset, world2texScale,
														brickTimeMin, brickTimeMax, time2texOffset, time2texScale))
				{
...
template<eAdvectMode advectMode, eTextureFilterMode filterMode>
__global__ void computeStreamFTLEKernel(SimpleParticleVertexDeltaT* dpParticles, VolumeInfoGPU c_volumeInfo, BrickIndexGPU c_brickIndex, BrickRequestsGPU c_brickRequests, uint particleCount, bool invertVelocity)
{
	uint index = blockIdx.x * blockDim.x + threadIdx.x;
...
#define INTEGRATE(advect, filter) computeStreamFTLEKernel <advect, filter> <<<blockCount, blockSize>>> (dpParticles, volumeInfo, brickIndex, brickRequests, particleCount, invertVelocity)
