#ifndef __TUM3D__ADVECT_DENSE_CPU_H__
#define __TUM3D__ADVECT_DENSE_CPU_H__
...
{
	//__device__ static inline bool exec(const TextureCPU<float4>& tex,
...
{
	static inline bool exec(const TextureCPU<float4>& tex,
		float3& pos, float& age, float3& vel, float& deltaTime,
		float3* pOutputCoeffs,
		...
		const float3 velocityScale)
	{
		...
		bool result = false;
		if(errorSquared < g_integrationParams.toleranceSquared || deltaTime <= g_integrationParams.deltaTimeMin) {
			pOutputCoeffs[0] = pos;
		...
		float scale = SCALE_SAFETY_FACTOR * pow(g_integrationParams.toleranceSquared / errorSquared, (1.0f / 5.0f) * 0.5f); // (tolerance/error)^(1/order)
		scale = clamp(scale, SCALE_MIN, SCALE_MAX);
		deltaTime *= scale;
		deltaTime = clamp(deltaTime, g_integrationParams.deltaTimeMin, g_integrationParams.deltaTimeMax);
...
template <eAdvectMode advectMode, eTextureFilterMode filterMode>
inline bool advectDense(const TextureCPU<float4>& tex,
	float3& pos, float& age, float3& vel, float& deltaTime,
	float3* pOutputCoeffs,
	const float3& world2texOffset, const float3 world2texScale,
	const float3 velocityScale)
