...
__device__ inline float3 getHeatCurrent(texture<float4, cudaTextureType3D, cudaReadModeElementType> tex, const float3& texCoord, float h)
{
...
template <eTextureFilterMode F>
__device__ inline float getHeatCurrentAlignment(texture<float4, cudaTextureType3D, cudaReadModeElementType> tex, const float3& texCoord, float h)
{
	float4 velT = sampleVolume<F, float4, float4>(tex, texCoord);
	float3 gradT = sampleScalarGradient<F>(tex, texCoord, h);
