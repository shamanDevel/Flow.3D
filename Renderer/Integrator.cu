...
void Integrator::IntegrateSimpleParticles(const BrickSlot& brickAtlas, SimpleParticleVertex* dpParticles, VolumeInfoGPU& volumeInfoGPU, BrickIndexGPU& brickIndex, BrickRequestsGPU& brickRequests, uint particleCount, const ParticleTraceParams& params)
{
	float timeMax = 1e10f;
...
void Integrator::UpdateIntegrationParams(const ParticleTraceParams& params, float timeMax, bool force)
{
	...
	integrationParamsGPU.cellChangeThreshold = params.m_cellChangeThreshold;
	integrationParamsGPU.timeInCellEnabled = params.m_seedTexture.m_colors != NULL;
