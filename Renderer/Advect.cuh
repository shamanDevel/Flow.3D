#ifndef __TUM3D__ADVECT_CUH__
#define __TUM3D__ADVECT_CUH__
...
struct advect_impl<ADVECT_HEUN, filterMode>
{
		...
		const float velocityScale)
	{
		...
		float3 velocity1 = getVelocity(pos + velocity0 * deltaTime);
...
struct advect_impl<ADVECT_RK3, filterMode>
{
	...
	{
		float3 velocity0 = vel;
		float3 velocity1 = getVelocity(pos + velocity0 * 0.5f * deltaTime);
		float3 velocity2 = getVelocity(pos + (velocity1 + velocity1 - velocity0) * deltaTime);
...
struct advect_impl<ADVECT_RK4, filterMode>
{
		...
		const float velocityScale)
	{
		float3 velocity0 = vel;
		float3 velocity1 = getVelocity(pos + velocity0 * 0.5f * deltaTime);
		float3 velocity2 = getVelocity(pos + velocity1 * 0.5f * deltaTime);
		float3 velocity3 = getVelocity(pos + velocity2 * deltaTime);
...
struct advect_impl<ADVECT_BS32, filterMode>
{
		...
		const float velocityScale)
	{
		...
		float3 vel3 = getVelocity(pos + deltaTime * (b32 * vel2));
		float3 deltaVel = deltaTime * (b41 * vel1 + b42 * vel2 + b43 * vel3);
		float3 vel4 = getVelocity(pos + deltaVel); // == vel1 in the next step (if step is accepted)
...
struct advect_impl<ADVECT_RKF45, filterMode>
{
		...
		const float velocityScale)
	{
		...
		float3 vel2 = getVelocity(pos + deltaTime * (b21 * vel1)); // k2 = (*f)(x0+h2, *y + h * b21 * k1);
		float3 vel3 = getVelocity(pos + deltaTime * (b31 * vel1 + b32 * vel2)); // k3 = (*f)(x0+h3, *y + h * ( b31*k1 + b32*k2) );
		float3 vel4 = getVelocity(pos + deltaTime * (b41 * vel1 + b42 * vel2 + b43 * vel3)); // k4 = (*f)(x0+h4, *y + h * ( b41*k1 + b42*k2 + b43*k3) );
		float3 vel5 = getVelocity(pos + deltaTime * (b51 * vel1 + b52 * vel2 + b53 * vel3 + b54 * vel4)); // k5 = (*f)(x0+h,  *y + h * ( b51*k1 + b52*k2 + b53*k3 + b54*k4) );
		float3 vel6 = getVelocity(pos + deltaTime * (b61 * vel1 + b62 * vel2 + b63 * vel3 + b64 * vel4 + b65 * vel5)); // k6 = (*f)(x0+h6, *y + h * ( b61*k1 + b62*k2 + b63*k3 + b64*k4 + b65*k5) );
...
__device__ inline bool advect(texture<float4, cudaTextureType3D, cudaReadModeElementType> tex,
	...
	const float velocityScale)
{
	return advect_impl<advectMode, filterMode>::exec(
		tex,
