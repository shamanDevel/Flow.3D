...
#define GEN_FEATURE_CASE(measure) case measure : GEN_FEATURE_KERNEL(measure); break
	...
	switch(params.m_measure1) 
	{
		...
		GEN_FEATURE_CASE(MEASURE_HEAT_CURRENT);
		GEN_FEATURE_CASE(MEASURE_HEAT_CURRENT_X);
		GEN_FEATURE_CASE(MEASURE_HEAT_CURRENT_Y);
		GEN_FEATURE_CASE(MEASURE_HEAT_CURRENT_Z);
